...
package Foswiki::Plugins::TableTmplPlugin::Core;
...
sub _appendRowNumberCssClass {
    ...
    my $rowClassName = $cssClasses{'tr-prefix'} . $colListName . $rowNum;
...
sub _appendColNumberCssClass {
    ...
    my $colClassName = $cssClasses{'col-prefix'} . $colNum;
...
sub _appendFirstColumnCssClass {
    ...
    return _appendToClassList( $classList, $cssClasses{'first-col'} );
}

sub _appendLastColumnCssClass {
    my ($classList) = @_;

    return _appendToClassList( $classList, $cssClasses{'last-col'} );
}

sub _appendLastRowCssClass {
    my ($classList) = @_;

    return _appendToClassList( $classList, $cssClasses{'last'} );
}

sub _appendSortedAscendingCssClass {
    my ($classList) = @_;

    return _appendToClassList( $classList, $cssClasses{'ascending'} );
}

sub _appendSortedDescendingCssClass {
    my ($classList) = @_;

    return _appendToClassList( $classList, $cssClasses{'descending'} );
}
...
sub _addStylesToHead {
    ...
    if ( defined $cssAttrs{dataBg} ) {
        unless ( $cssAttrs{dataBg} =~ /none/i ) {
            ...
            foreach my $color (@attrDataBg) {
                next if !$color;
                my $rowSelector = $cssClasses{'tr-prefix'} . 'dataBg';
                $rowSelector .= $count;
    ...
    if ( defined $cssAttrs{dataColor} ) {
        unless ( $cssAttrs{dataColor} =~ /none/i ) {
            ...
            foreach my $color (@attrDataColor) {
                next if !$color;
                my $rowSelector = $cssClasses{'tr-prefix'} . 'dataColor';
                $rowSelector .= $count;
                my $attr = 'color:' . $color . ';';
                push( @styles, "$selector tr.$rowSelector td {$attr}" );
    ...
    if ( defined $cssAttrs{columnWidths} ) {
        ...
        foreach my $width (@attrColumnWidths) {
            ...
            my $colSelector = $cssClasses{'col-prefix'};
            $colSelector .= $count;
            my $attr = 'width:' . $width . ';';
...
sub emitTable {
    ...
    my $tattrs = { class => $cssClasses{'table'} };
...
sub handler {
    ...
    unless ($Foswiki::Plugins::TableTmplPlugin::initialised) {
        $insideTABLE = 0;
        $tableCount  = 0;
        ...
        $Foswiki::Plugins::TableTmplPlugin::initialised = 1;
    }
